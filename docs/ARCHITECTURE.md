...
- **`models/`** — değer nesneleri: `PrayerTime`, `Location`, `NotificationSetting`. `fromJson`/`toJson` içerir.
...
| `location` | `turkey_locations_data` | `location_repository`, `location_service`, `location_monitor_service` |
...
Uygulamanın geri kalanı `PrayerTimeProvider` arayüzüne bağlı olduğu için değişmeden çalışır.
