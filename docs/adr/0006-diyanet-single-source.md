...
- **Vakitler ilçe-yıl dosyası olarak** çekilir (`DiyanetProvider`, `GET /v1/prayer-times/{cityId}/{year}`, `If-None-Match`/304 — `diyanet_provider.dart:84-94`), gün gün önbelleğe Hicri tarihle birlikte yazılır. Depo sözleşmesi değişmedi; sağlayıcı arayüzü `Location` alır ve `cityId`'yi oradan okur.
